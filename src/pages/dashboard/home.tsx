...
import { FileText, Sparkles, Plus, ArrowRight, Brain, ChevronRight, Clock, Zap } from 'lucide-react'
...
export default function DashboardHome() {
  ...
  const [stats, setStats] = useState({
    ...
    cardCount: 0,
    interviewCount: 0,
    avgInterviewScore: null as number | null,
  })
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const avgScore = interviews && interviews.length > 0
        ? Math.round(interviews.reduce((sum: number, i: any) => sum + (i.overall_score || 0), 0) / interviews.length)
        : null

      setStats({
        resumeCount: resumeCount || 0,
        customizedCount: customizedCount || 0,
      ...
      <div className="space-y-8">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-violet-600 via-indigo-600 to-purple-700 p-6 md:p-8 text-white">
          ...
          <div className="relative z-10 flex items-center justify-between">
            ...
            <Link to="/resumes/new" className="hidden md:block">
          ...
          {!isNewUser && (
            <div className="relative z-10 mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <Link to="/customize" className="block group">
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 hover:bg-white/20 transition-all cursor-pointer">
                  <p className="text-white/70 text-xs font-medium">AI Tailored</p>
                  <p className="text-2xl font-bold">{stats.customizedCount}</p>
                </div>
              </Link>
              ...
              <Link to="/interview" className="block group">
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 hover:bg-white/20 transition-all cursor-pointer">
                  ...
                  <p className="text-2xl font-bold">{stats.avgInterviewScore || '—'}</p>
                </div>
              </Link>
            </div>
          )}
        ...
        <div className="mb-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <Link to="/resumes/new" className="block group">
